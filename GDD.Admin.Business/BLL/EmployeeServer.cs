...
namespace GDD.Admin.Business.BLL
{
    ...
    public class EmployeeServer : Repository, IEmployeeService
    {
        ...
        /// <returns></returns>
        public List<Employee> GetEmployeeList(string name, int pageIndex, int pageSize)
        {
            using (var db = base.GDDSVSPDb)
            {
        ...
        /// <returns></returns>
        public List<SubmittedEmployee> GetSubmittedEmployeeList(string name,Guid? departmentId, Guid? questionnaireId, int pageIndex, int pageSize, int isSubmit)
        {
            using (var db = base.GDDSVSPDb)
            {
        ...
        /// <param name="isSubmit">是否已提交  0：否  1：是</param>
        /// <returns></returns>
        public int GetSubmittedEmployeeCount(string name, Guid? departmentId, Guid? questionnaireId, int isSubmit)
        {
            using (var db = base.GDDSVSPDb)
            {
                IQueryable<Employee> employees = db.Employee;
        ...
        /// <param name="departmentId">部门ID</param>
        /// <returns></returns>
        public int GetEmployeeCount(string name,Guid? departmentId)
        {
            using (var db = base.GDDSVSPDb)
            {
                IQueryable<Employee> baseQuery = db.Employee;
